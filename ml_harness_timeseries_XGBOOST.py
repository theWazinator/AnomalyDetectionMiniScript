...
def get_results(dvalid, validation_target_df, model, model_params, save_folder, t_num, time_elapsed, validation_record_table_df, feature_set_filename):
    ...
    joined_df['predicted_censored_value'] = model.predict(dvalid)  # note - this changes with every model type
    joined_df['predicted_censored_threshold'] = 0.5  # note - this changes with every model type
...
def run_ml_model(training_descriptive_df, training_target_df, validation_descriptive_df, validation_target_df, model_params, save_folder, t_num, validation_record_table_df, feature_set_filename):
    ...
    results_df = get_results(dvalid, validation_target_df, model, model_params, save_folder, t_num, time_elapsed, validation_record_table_df, feature_set_filename)
...
version = "Satellite_Timing_Experiments_new_V5"
    ...
    for starting_train_month in range(7, 14-train_month_count):
        ...
        for test_month in range(train_month_range[-1]+1, 14):
            ...
            for training_month in train_month_range:
                ...
                training_descriptive_file_name = home_file_name + country_code + "/ML_ready_dataframes_V5/" +str(og_train_month)+ "_" +str(og_train_year)+ "/TRAINING_Mixed_descriptiveFeatures_fullDataset.gzip"
                ...
                training_target_file_name = home_file_name + country_code + "/ML_ready_dataframes_V5/" + str(og_train_month) + "_" + str(og_train_year) + "/TRAINING_Mixed_targetFeature_GFWatch_Censored.csv"
            ...
            test_descriptive_file_name = home_file_name + country_code + "/ML_ready_dataframes_V5/" +str(og_test_month)+ "_" +str(og_test_year)+ "/TESTING_Mixed_descriptiveFeatures_fullDataset.gzip"
            test_target_file_name = home_file_name + country_code + "/ML_ready_dataframes_V5/" +str(og_test_month)+ "_" +str(og_test_year)+ "/TESTING_Mixed_targetFeature_GFWatch_Censored.csv"
            validation_record_table = home_file_name + country_code + "/ML_ready_dataframes_V5/" + str(
                og_test_month) + "_" + str(og_test_year) + r'/TESTING_Mixed_record_summary_table.csv'
            ...
            training_descriptive_df, test_descriptive_df, test_target_df, validation_comparison_df = \
                remove_features(training_descriptive_df, test_descriptive_df, test_target_df, test_target_df, features_to_remove)
            ...
            run_ml_model(training_descriptive_df, training_target_df, test_descriptive_df, test_target_df,  model_params, save_folder, t_name, validation_record_table_df, feature_set_filename)
